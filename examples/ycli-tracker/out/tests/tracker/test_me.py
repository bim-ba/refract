...
_URL_get = "https://api.tracker.yandex.net/v3/myself"
_PAYLOAD_get = {"uid": 42, "login": "alice", "display": "Alice A.", "email": "alice@example.com"}
_runner = CliRunner()
...
async def test_me_mcp_auth_guard(creds):
    responses.add(responses.GET, _URL_get, json={}, status=401)
...
async def test_me_mcp_empty_response_guard(creds):
    ...
    responses.add(responses.GET, _URL_get, json={}, status=200)
    async with Client(me_mcp_module.mcp) as client:
        with pytest.raises(ToolError):
            await client.call_tool("me_get", {})
