...
def _client_method(
    ...
    doc = doc_comments.render(op.documentation, "    ")
...
def resolve_client(
    ...
    for op in res.operations:
        text, method_imports = _client_method(op, naming, type_mapper, doc_comments)
        methods.append(text)
        imports += method_imports
    return ClientPageView(
        doc_block=doc_comments.render(res.module_docs.client, ""),
        import_lines=render_imports(tuple(imports)),
        class_header=f"class {naming.class_name(res.resource, 'Client')}({base_class}):",
        class_doc_lines=doc_comments.render(res.module_docs.client_class, "    "),
