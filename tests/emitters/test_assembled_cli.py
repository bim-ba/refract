...
def test_assembled_options_flatten_one_level_ref():
    ...
    op = res.operations[0]
    decls, expr, imports = _assembled_options(res, op, CTX)
    joined = " ".join(decls)
...
def test_assembled_options_dangling_ref_raises_specerror_not_keyerror():
    ...
    with pytest.raises(SpecError, match="undeclared model 'Nope'"):
        _assembled_options(res, res.operations[0], CTX)
...
def test_assembled_options_guards_shadowed_body_field():
    ...
    res = _single_body_resource(model)
    decls, expr, _imports = _assembled_options(res, res.operations[0], CTX)
...
def test_cli_command_guards_shadowed_body_field_end_to_end():
    ...
    block, _imports = _cli_command(res, op, CTX)
    ast.parse(block)
...
def test_cli_command_write_op_threads_path_and_query_params():
    ...
    block, _imports = _cli_command(res, edit, CTX)
...
def test_cli_command_read_op_unchanged():
    ...
    block, imports = _cli_command(res, get_op, CTX)
    assert imports == []
    assert block == (
        "@app.command()\n"
